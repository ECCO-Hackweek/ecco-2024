...
"""
This file contains commonly used routines for manipulating SWOT SSH data.
...
def search_cmr(short_name, bounding_box, time_range):
    """
    Search granule S3 filenames
    
    Example usage:

    search_cmr("SENTINEL-1A", "-10,-10,10,10", "2022-01-01T00:00:00Z,2022-01-31T23:59:59Z")
...
def get_mss22(longitude, latitude):
    ...
    if 'jovyan' in os.getcwd():
        msspth='/home/jovyan/efs/SWOT_shared/data/MSS/mss_cls22_updated.grd'
    else:
        msspth='/mnt/flow/swot/MSS/MSS_2022/mss_cls22_updated.grd'
    mss = open_dataset(msspth)
...
def resample_swath_data(lon, lat, ssh, lon_out, lat_out, 
                        sigma=1000, 
                        radius_of_influence = 6000,
                       resample_type='gaussian'):
    """
    ...
    if resample_type=='gaussian':
        ...
        data_out = kd_tree.resample_nearest(
            input_grid, ssh, output_grid,
            radius_of_influence=radius_of_influence,
            fill_value=None
        )
